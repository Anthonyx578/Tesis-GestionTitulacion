...
const MemoizedLink = React.memo(function MemoizedLink({ item, menu }: { item: { title: string; url: string; imagen: JSX.Element }; menu: boolean }) {
    return (
      <Link href={item.url} className={`flex items-center ${menu ? 'justify-start px-3 py-2' : 'justify-center p-2'} gap-2 hover:bg-black/10 dark:hover:bg-white/10 hover:border hover:border-black/30 dark:hover:border-white/70 rounded transition-colors duration-200`}>
        ...
        <div className={`${menu ? 'w-10/12 absolute sm:relative sm:w-4/12 md:w-4/12 lg:w-2.50/12' : 'w-16'} bg-primary_white text-sm dark:bg-primary_dark transition-all duration-300 flex flex-col border-r border-black/10 h-screen truncate text-black/90 dark:text-white/90 z-20`}>
            ...
            <div className="flex flex-col justify-between h-full p-2">
                ...
                <div className="flex flex-col gap-4">
                    <button onClick={toggleDarkMode} className={`flex items-center ${menu ? 'justify-start px-3 py-2' : 'p-2 justify-center'} gap-2 hover:bg-black/10 dark:hover:bg-white/10 hover:border hover:border-black/30 dark:hover:border-white/70 rounded transition-colors duration-200`}>
                        {darkMode ?
