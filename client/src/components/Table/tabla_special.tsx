...
export default function TableComponent({title, addView , datoView}: {title:string; addView:boolean; datoView:Number}){
    ...
    const handlePageChange = (page: number) => {router.push(`${pathname}?page=${page}`);};
    ...
    const renderPagination = () => {
        ...
        if (pag_actual > 1) {
            buttons.push(<button key={1} onClick={() => handlePageChange(1)} className="p-2 border dark:border-white/10 border-black/10 rounded-md" aria-label="Ir a la primera página">1</button>);
        }

        if (pag_actual > 3) {
            buttons.push(<span key="ellipsis-start" className="p-2 border border-black/10 dark:border-white/10  rounded-md">...</span>);
        }

        if (pag_actual > 2) {
            buttons.push(<button key={pag_actual - 1} onClick={() => handlePageChange(pag_actual - 1)} className="p-2 border dark:border-white/10 border-black/10 rounded-md">{pag_actual - 1}</button>);
        }

        buttons.push(<button key={pag_actual} className="p-2 border border-black/10 dark:border-white/10 dark:bg-white/30 bg-black/10 rounded-md" disabled>{pag_actual}</button>);
        ...
        if (pag_actual < pag_total - 2) {
            buttons.push(<span key="ellipsis-end" className="p-2 border border-black/10 dark:border-white/10  rounded-md">...</span>);
        }

        if (pag_actual < pag_total) {
            buttons.push(<button key={pag_total} onClick={() => handlePageChange(pag_total)} className="p-2 border dark:border-white/10 border-black/10 rounded-md" aria-label={`Ir a la última página ${pag_total}`}>{pag_total}</button>);
        }
        ...
        <div className="flex flex-col gap-3 p-4 md:p-2 lg:p-2">
            <div className="flex flex-col md:flex-row justify-between items-center">
                <div className="flex flex-row gap-4 items-center">
                    ...
                    {addView && (
                        <Link href={`${pathname}/formulario`} className='p-1 bg-blue_personalizado/80 hover:bg-blue_personalizado hover:shadow-sm hover:shadow-blue-300/20 rounded-md'>
                ...
                <div className="flex flex-row items-center gap-2 mt-3 md:mt-0">
                    <div className="flex flex-row items-center relative rounded-sm shadow-sm bg-black/10 dark:bg-white/90 border border-black/30 text-black/80">
                        <Image src={ISearch} alt="Icono Buscar" width={28} height={20} className="absolute p-1" />
                        <input type="text" className="pl-7 pr-2 py-1 rounded-sm text-gray-800 w-full md:w-auto" placeholder="Búsqueda"/>
                    </div>
                </div>
            </div>
            ...
            <div className="flex items-center justify-center mt-4 gap-2">
                {pag_actual > 1 && (
                    <button onClick={() => handlePageChange(pag_actual - 1)} className="p-2 border dark:border-white/10 border-black/10 rounded-md font-semibold" aria-label="Página anterior">&lt;</button>
                )}

                {paginationButtons}

                {pag_actual < pag_total && (
                    <button onClick={() => handlePageChange(pag_actual + 1)} className="p-2 border dark:border-white/10 border-black/10 rounded-md" aria-label="Página siguiente">&gt;</button>
                )}
            </div>
        </div>
    );
