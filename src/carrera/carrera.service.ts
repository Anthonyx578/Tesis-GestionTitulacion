...
export class CarreraService {
  constructor(
    @InjectRepository(carrera) private repositoty: Repository<carrera>,
  ) {}
  ...
  async Get(id: number) {
    try {
      return await this.repositoty.findOne({
        where: { id_carrera: id, status: 1 },
        select: ['id_carrera', 'nombre_carrera'],
      });
    } catch (e) {
      throw new RpcException(e);
  ...
  async update(id: number, ChangeData: carreraDTO) {
    try {
      const ExistData = await this.repositoty.findOne({
        where: { id_carrera: id, status: 1 },
      });
      if (!ExistData) {
        return {};
      }
      return this.repositoty.update(
        { id_carrera: id },
  ...
  async delete(id) {
    try {
      const Exist = this.repositoty.findOne({
        where: { id_carrera: id, status: 1 },
      });
      if (!Exist) {
