...
export class SustentacionService {
  ...
  async GetAllByPeriod(Periodo: string) {
    try {
      const Sustentaciones = await this.repository.find({
        where: { periodo_academico: Periodo, status: 1 },
        select:['id_sustentacion','id_tesis','id_carrera','periodo_academico','fecha_sustentacion','aula','tipo']
      });
