...
export class UsuarioService {
    ...
    async update (id:number,ChangeData:UsuarioUpdateDTO){
        try{
            const ExistData = await this.repository.findOne({where:{id_usuario:id,status:1}})
            if(!ExistData){
                return {}
            }
    ...
    async delete (id){
        try{
            const Exist = this.repository.findOne({where:{id_usuario:id,status:1}})
            if(!Exist){
                return {message:"Dato no encontrado o ya eliminado"}
            }
            return this.repository.update({id_usuario:id},{deleted_at:new Date(),status:0})
        }
        catch(e){
            throw new RpcException(e)
        }
    ...
    async restore (id){
        try{
            const Exist = this.repository.findOne({where:{id_usuario:id,status:0}})
            if(!Exist){
                return {message:"Dato no encontrado o ya restaurado"}
            }
            return this.repository.update({id_usuario:id},{deleted_at:null,status:1})
        }
        catch(e){
            throw new RpcException(e)
        }
    ...
    async FindbyUser (nombre_usuario:string){
        return await this.repository.findOne({where:{nombre_usuario:nombre_usuario,status:1},select:['id_usuario','nombre_usuario','contrasena','id_carrera','id_rol']})
    }
